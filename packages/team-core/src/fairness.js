...
const playerScore = (skillLike) => skillLike.averageScore ?? skillLike.skillScore ?? 0;
...
const computeTeamAverageSkill = (team) => {
    if (team.members.length === 0)
        return 0;
    return average(team.members.map((member) => playerScore(member)));
};
export const calculateFairnessMetrics = (teams, enforceGenderBalance) => {
    if (teams.length === 0) {
        return {
            averageDeviation: 0,
            standardDeviation: 0,
    ...
    if (enforceGenderBalance) {
        ...
        genderDistributionValid = teams.every((team) => {
            ...
            return Object.keys(teamCounts).every((gender) => {
    ...
    const combinedPenalty = averageDeviation * 0.4 + standardDeviation * 0.3 + highestLowestGap * 0.3;
