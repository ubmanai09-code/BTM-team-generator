...
export const generationConstraintsSchema = z.object({
  ...
  femalePerTeam: z.number().int().min(1).max(10).default(1),
  ...
  if (value.enforceFemalePerTeam && value.femalePerTeam > value.teamSize) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["femalePerTeam"],
      message: "femalePerTeam cannot exceed teamSize"
    });
  }
});
...
export const teamGenerationRequestSchema = teamGenerationRequestBaseSchema.superRefine((value, ctx) => {
  ...
  if (value.constraints.enforceFemalePerTeam && value.constraints.teamSize > 0) {
    ...
    if (femaleCount < value.constraints.femalePerTeam) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["participants"],
        message: `Impossible scenario: at least ${value.constraints.femalePerTeam} female players are required`
      });
    }
  }
});
