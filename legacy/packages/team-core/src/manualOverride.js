...
const participantScore = (participant) => participant.averageScore ?? participant.skillScore ?? 0;
const cloneTeams = (teams) => teams.map((team) => ({ ...team, members: [...team.members] }));
const findTeamOrThrow = (teams, teamId) => {
    const team = teams.find((candidate) => candidate.id === teamId);
    if (!team)
        throw new DomainError(`Team ${teamId} not found`);
    return team;
};
export const applyManualOverride = (request, enforceGenderBalance) => {
    ...
    for (const operation of request.operations) {
        ...
        const sourceIndex = fromTeam.members.findIndex((member) => member.id === operation.participantId);
        if (sourceIndex === -1) {
            throw new DomainError(`Participant ${operation.participantId} not found in ${fromTeam.id}`);
        }
        ...
        const targetIndex = toTeam.members.findIndex((member) => member.id === operation.swapWithParticipantId);
        if (targetIndex === -1) {
            throw new DomainError(`Participant ${operation.swapWithParticipantId} not found in ${toTeam.id}`);
        }
    ...
    const analytics = request.baseResult.analytics.map((teamAnalytics) => {
        ...
        const divisions = team.members.reduce((acc, member) => {
...
//# sourceMappingURL=manualOverride.js.map